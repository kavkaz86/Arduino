...
void setup() {
  ...
  pinMode(A1, INPUT); //MQ-8 HYDROGEN
  //pinMode(A2, INPUT);
  //pinMode(A3, INPUT);
  //pinMode(A4, INPUT);
...
void loop() {
   ...
   // Humidity 
    inByte = analogRead(A0); //ADC 
    inByte = 5*inByte/1024; //Voltage
    inByte = (inByte - 0.958)/0.0307; //RH% 
    float2String(inByte, inString, sizeof(inByte));   
    payload[0] = 'H';
    payload[1] = inString[0]; 
    payload[2] = inString[1]; 
    ...
    payload[4] = inString[3]; 
    payload[5] = inString[4];  
    mySerial << _BYTE(payload[1]) << _BYTE(payload[2]) << _BYTE(payload[3]) << _BYTE(payload[4]) << _BYTE(payload[5]) << endl;
    //xbee.send(zbTx);    
    inByte = 0; 
    delay(2000);
    ...
    inByte = analogRead(A1); //ADC
    inByte = map(inByte, 0,1023,0,100); //Percentage % 
    float2String(inByte, inString, sizeof(inByte));     
    payload[0] = 'M';
    payload[1] = inString[0]; 
    payload[2] = inString[1];  
