...
void loop() {
  ...
  while (!IR_alarm && !rep_flag && !diag_flag) {
    ...
    //Serial.println(DUST_vals[0]);
    //Serial.println(DUST_vals[1]);
  ...
  if (rep_flag) {
    repCount = 0;
    irmap = IR_s.IR_mapS(IR_flag);
    comms.sendTX(repF, NODE_ID[0], irmap, REP_vals, 6);
    rep_flag = false;
  }
 ...
 * b. Reset data arrays.
 */
void clearVals(int type, const bool IR[]) {
...
void cleanUp() {
  ...
  clearVals(zIR_vals, zIR);
  clearVals(zIR_flag, zIR);
  clearVals(zRep_flag, zRep);
