 ...
 */
static const byte Dust_sensor[2] = {18, A5};
 ...
 */
static const byte Comms_xbee = 14;
...
int detect_task() {
  ...
  if (check_airDensity(DUST_vals, 0.5)) {
    AD_alarm = true;
    return 1;
  }
  ...
  else if (sleepCount >= 200) {
    sleepCount = 0;
    CO2_vals[0] = CO2_s1.calc_CO2_v();
    CO2_vals[1] = CO2_s1.calc_CO2_percent(CO2_vals[0]);

    if (check_gasPPM(CO2_vals, 400)) {
      Gas_alarm = true;
      if (heaterStatus) {
        CO2_s1.setGasHeater(false, 1);
        heaterStatus = false;
        return 1;
      }
    }
