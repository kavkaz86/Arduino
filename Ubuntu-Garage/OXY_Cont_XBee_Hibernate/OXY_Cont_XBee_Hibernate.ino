...
void setup()
{
  ...
  myserial.print("%"); delay(100); myserial.print('\r'); delay(100);
  
  tempread(); // Take initial temperature reading
...
void loop()
{
  ...
  //sensorstring ="O:"; // Format output string 
  delay(15000); // 15-seconds required for radio reassociation 
  
   // Take a temperature measurement every 80 seconds and update the Oxygen Temp Variable 
    if(temp_counter == 10){
     temp_counter = 0 ; 
     celsius = 0;
     tempread();
  ...
  while(cont_count < 10){
    ...
    while (myserial.available()) {
      char inchar = (char)myserial.read();
       ...
       if (inchar == '\r') { cont_count++; 
                             Serial << "O:" << sensorstring; delay(100); 
                             sensorstring = "";  
                           }  
    }
  }
